...
export function EmployeeDetailPage() {
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      ...
      apiGetConnectionHistory(id).then(ch => { if (!cancelled) setConnectionHistory(ch); }).catch(() => {});
      apiGetKeylogHistory(id).then(kh => { if (!cancelled) setKeylogHistory(kh); }).catch(() => {});
      apiGetFileActivity(id).then(fa => { if (!cancelled) setFileActivity(fa); }).catch(() => {});
      apiGetPrintLogs(id).then(pl => { if (!cancelled) setPrintLogs(pl); }).catch(() => {});
    };

    loadData();

    // Auto-refresh every 30s for live updates (screenshots, alerts, etc)
